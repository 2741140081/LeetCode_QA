package com.marks.leetcode.DP;

import java.util.HashMap;
import java.util.Map;

/**
 * <p>项目名称: LeetCode_QA </p>
 * <p>文件名称: LeetCode_403 </p>
 * <p>描述: [类型描述] </p>
 *
 * @author marks
 * @version v1.0
 * @date 2025/11/12 11:39
 * @update [序号][日期YYYY-MM-DD] [更改人姓名][变更描述]
 */
public class LeetCode_403 {

    /**
     * @Description:
     * 一只青蛙想要过河。
     * 假定河流被等分为若干个单元格，并且在每一个单元格内都有可能放有一块石子（也有可能没有）。
     * 青蛙可以跳上石子，但是不可以跳入水中。
     *
     * 给你石子的位置列表 stones（用单元格序号 升序 表示）， 请判定青蛙能否成功过河（即能否在最后一步跳至最后一块石子上）。
     * 开始时， 青蛙默认已站在第一块石子上，并可以假定它第一步只能跳跃 1 个单位（即只能从单元格 1 跳至单元格 2 ）。
     *
     * 如果青蛙上一步跳跃了 k 个单位，那么它接下来的跳跃距离只能选择为 k - 1、k 或 k + 1 个单位。
     * 另请注意，青蛙只能向前方（终点的方向）跳跃。
     * @param: stones
     * @return boolean
     * @author marks
     * @CreateDate: 2025/11/12 11:41
     * @update: [序号][YYYY-MM-DD] [更改人姓名][变更描述]
     */
    public boolean canCross(int[] stones) {
        boolean result;
        result = method_01(stones);
        result = method_02(stones);
        return result;
    }

    private Boolean[][] dp;
    private Map<Integer, Integer> map;
    private boolean method_02(int[] stones) {
        int n = stones.length;
        dp = new Boolean[n][n];
        map = new HashMap<>();
        for (int i = 0; i < stones.length; i++) {
            map.put(stones[i], i);
        }
        return dfs(stones, 0, 0);
    }

    private boolean dfs(int[] stones, int i, int lastDis) {
        if (i == stones.length - 1) {
            return true;
        }
        if (dp[i][lastDis] != null) {
            return dp[i][lastDis];
        }
        for (int k = lastDis - 1; k <= lastDis + 1; k++) {
            if (k > 0) {
                int j = map.get(stones[i] + k);
                if (j >= 0 && dfs(stones, j, k)) {
                    return dp[i][lastDis] = true;
                }
            }
        }
        return dp[i][lastDis] = false;
    }

    /**
     * @Description:
     * 输入：stones = [0,1,3,5,6,8,12,17]
     * 输出：true
     * k = 1,
     * 1. 如果要进行动态规划, 如何存储 boolean[][] dp = new boolean[stones.length][k],
     * k 表示当前位置的跳跃距离, k 的取值范围是[1, stones.length];
     * 2. dp[1][1], dp[1][2] 分别能跳到什么位置
     * AC: 28ms/
     * @param: stones
     * @return boolean
     * @author marks
     * @CreateDate: 2025/11/12 11:41
     * @update: [序号][YYYY-MM-DD] [更改人姓名][变更描述]
     */
    private boolean method_01(int[] stones) {
        int n = stones.length;
        boolean[][] dp = new boolean[n][n];
        dp[0][0] = true;
        for (int i = 1; i < n; i++) {
            if (stones[i] - stones[i-1] > i) {
                return false;
            }
        }
        for (int i = 1; i < n; i++) {
            for (int j = i - 1; j >= 0; j--) {
                int k = stones[i] - stones[j];
                if (k > j + 1) {
                    break;
                }
                dp[i][k] = dp[j][k-1] || dp[j][k] || dp[j][k+1];
                if (i == n-1 && dp[i][k]) {
                    return true;
                }
            }
        }
        return false;
    }

}
