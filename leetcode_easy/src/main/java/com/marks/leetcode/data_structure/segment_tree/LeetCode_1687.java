...
public class LeetCode_1687 {
     ...
     * @return int
     * @author marks
     * @CreateDate: 2026/06/17 9:57
     * @update: [序号][YYYY-MM-DD] [更改人姓名][变更描述]
     */
    public int boxDelivering(int[][] boxes, int portsCount, int maxBoxes, int maxWeight) {
        int result;
        result = method_01(boxes, portsCount, maxBoxes, maxWeight);
        return result;
    }

    /**
     * @Description:
     * 1. 线段树的作用是查询区间的相关结果, 例如元素和, 最大值, 最小值, 平均值等等。
     * 2. 好像无法使用线段树, 查看题解, 使用动态规划 + 单调队列优化, 直接 CV 题解
     * @param: boxes
     * @param: portsCount
     * @param: maxBoxes
     * @param: maxWeight
     ...
     * @update: [序号][YYYY-MM-DD] [更改人姓名][变更描述]
     */
    private int method_01(int[][] boxes, int portsCount, int maxBoxes, int maxWeight) {
        int n = boxes.length;
        int[] p = new int[n + 1];
        int[] w = new int[n + 1];
        int[] neg = new int[n + 1];
