...
public class LeetCode_174 {
    ...
    public int calculateMinimumHP(int[][] dungeon) {
        int result = 0;
        result = method_01(dungeon);
        return result;
    }
    /**
     * dp[i][j] 存储 坐标(i, j)到终点所需的最小值
     * 由坐标(i, j)可以到达的下一个左边分别是(i+1, j) (i, j+1)
     * dp[i][j] = Math.max( Math.min(dp[i+1][j], dp[i][j+1]) - dungeon[i][j], 1)
     */
    private int method_01(int[][] dungeon) {
        ...
        for (int i = m - 1; i >= 0; i--) {
            for (int j = n - 1; j >= 0; j--) {
                ...
                dp[i][j] = Math.max(minValue - dungeon[i][j], 1);
            }
        }
        return dp[0][0];
    }
